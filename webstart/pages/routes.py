        ...
        if "token" in session:
            ...
            if db.user_data.find_one({"_id": current_user.id}) is not None:
                return render_template(
                    "socials.html", current_user=current_user, authenticated_user=True
                )
            return render_template("socials.html", current_user=current_user)
    except BadTokenError:
        return render_template(
            "socials.html", oauth_url=_blueprint_config_data.OAUTH_URL
        )
...
def premium():
    try:
        if "token" in session:
            ...
            if db.user_data.find_one({"_id": current_user.id}) is not None:
                return render_template(
                    "premium.html", current_user=current_user, authenticated_user=True
                )
            return render_template("premium.html", current_user=current_user)
    except BadTokenError:
        return render_template(
            "premium.html", oauth_url=_blueprint_config_data.OAUTH_URL
        )
...
def patch_notes(version):
    if version == "latest-release":
        ...
        patch_notes = [
            patch_note
            for patch_note in db.patch_notes.find().sort("Version", -1).limit(5)
        ]
    ...
    try:
        if "token" in session:
            ...
            return render_template(
                "patch_notes.html", current_user=current_user, patch_notes=patch_notes
            )
    except BadTokenError:
        return render_template(
            "patch_notes.html",
...
def devsinfo():
    try:
        if "token" in session:
            ...
            if db.user_data.find_one({"_id": current_user.id}) is not None:
                return render_template(
                    "devinfo.html", current_user=current_user, authenticated_user=True
                )
            return render_template("devinfo.html", current_user=current_user)
    except BadTokenError:
        return render_template(
            "devinfo.html", oauth_url=_blueprint_config_data.OAUTH_URL
        )
...
def test():
    try:
        if "token" in session:
            ...
            if db.user_data.find_one({"_id": current_user.id}) is not None:
                return render_template(
                    "test.html", current_user=current_user, authenticated_user=True
                )
